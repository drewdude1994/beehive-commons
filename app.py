...
def index():
    ...
    return jsonify({"status": "Beehive API WITH PAYPAL FINAL v4", "paypal_enabled": paypal_enabled()})
@app.route("/api/platform_stats")
def platform_stats():
    conn = get_conn()
    plat = conn.execute("SELECT * FROM operators WHERE id='hive_platform'").fetchone()
    payouts = [dict(r) for r in conn.execute("SELECT * FROM paypal_payouts WHERE type='platform' AND status='sent'").fetchall()]
    total = sum([float(p["net_amount"] or p["fee_amount"] or 0) for p in payouts])
    if plat and plat["usd_earned"]:
        total = max(total, plat["usd_earned"])
    all_payouts = [dict(r) for r in conn.execute("SELECT * FROM paypal_payouts").fetchall()]
    jobs = len(set([p["need_id"] for p in all_payouts if p["need_id"]]))
    conn.close()
    return jsonify({"total_earned": total, "total_jobs": jobs, "fee_pct": PLATFORM_FEE_PCT, "fee_display": "1.4%", "paypal_email": PLATFORM_PAYPAL_EMAIL, "example": {"10_job_fee": 0.14, "10_job_worker": 9.86}})
@app.route("/api/config")
def config():
    return jsonify({"HC_TO_USD_RATE": HC_TO_USD_RATE, "PLATFORM_FEE_PCT": PLATFORM_FEE_PCT, "PLATFORM_FEE_DISPLAY": "1.4%", "PLATFORM_PAYPAL_EMAIL": PLATFORM_PAYPAL_EMAIL, "PAYPAL_MODE": PAYPAL_MODE, "PAYPAL_ENABLED": paypal_enabled(), "RATE": f"1 HC = ${HC_TO_USD_RATE}", "FINAL": "v4"})
...
def create_need():
    ...
    return jsonify({"id": nid, "barter_value": value, "fee_hc": fee_hc, "fee_usd": fee_usd, "net_hc": net_hc, "net_usd": round(net_hc*HC_TO_USD_RATE,2), "gross_usd": round(value*HC_TO_USD_RATE,2)})
...
def create_operator():
    ...
    if not paypal_email: return jsonify({"error": "PayPal email required - Use REAL PayPal Email"}), 400
    conn = get_conn()
    ...
    conn.execute("INSERT INTO operators (id, name, pools, hc_balance, usd_earned, zone, password, paypal_email, created_at) VALUES (?,?,?,?,?,?,?,?,?)", (op_id, name, json.dumps(["real_world_ops"]), 100.0, 0.0, "46250", password, paypal_email, datetime.utcnow().isoformat())); conn.commit(); conn.close(); return jsonify({"ok": True, "id": op_id, "paypal_email": paypal_email})
...
if __name__ == "__main__":
    print("BEEHIVE PAYPAL LIVE FINAL v4"); app.run(host="0.0.0.0", port=5000, debug=True)
